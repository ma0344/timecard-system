<?php
/*
 * 目的: 有給申請に対する本人向け決裁（承認/却下）処理を行います。
 * 入力: request_id、decision（approve/reject）、コメント等
 * 出力: 処理結果（成功/失敗）と通知生成
 */
?>
<?php
...
list($lim, $win) = rate_cfg($pdo, 'leave_requests_decide', 10, 60);
if (!rate_limit($pdo, 'leave_requests_decide', $lim, $win)) {
...
try {
  ...
  $stmt = $pdo->prepare('SELECT id, user_id, used_date, hours, status, approve_token_expires_at FROM leave_requests WHERE (approve_token = ? OR approve_token_hash = ?) LIMIT 1');
  ...
  if ($newStatus === 'approved') {
    ...
    $pdo->prepare('INSERT INTO paid_leave_use_events (user_id, used_date, total_hours, reason) VALUES (?, ?, ?, ?)')
      ->execute([$userId, $usedDate, $usedHours, '承認により自動反映']);
    ...
    if ($needed > 1e-9 && !$allowNegative) {
      // 残が不足かつ許可なし→承認は維持しつつイベントは未充当分をマイナス扱いで記録
      // 記録のみ（残高には反映しない）
    }
    if ($needed > 1e-9) {
