<?php
/*
 * 目的: 付与済み有給の削除を行います（誤付与の是正）。
 * 入力: grant_id
 * 出力: 削除結果
 */
?>
<?php
// api/paid_leave_grant_delete.php
// 管理者が付与(paid_leaves)を削除するAPI。消化済みの付与は削除不可。削除後は再計算。
session_start();
...
try {
  ...
  $stmt = $pdo->prepare('SELECT user_id, consumed_hours_total FROM paid_leaves WHERE id = ? FOR UPDATE');
  ...
  if ($consumed > 1e-9) {
    throw new Exception('cannot delete: grant already consumed');
  }

  $pdo->prepare('DELETE FROM paid_leaves WHERE id = ?')->execute([$grantId]);
