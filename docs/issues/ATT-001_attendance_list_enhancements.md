...
- 欠落行（未打刻）でのクイック操作: 「全休」ボタンを追加し、即時に全休（override=off_full）へ切替できるようにする（新規）。
...
- 現行の出勤日数は「実記録（出退勤）ありの日」をカウント。仕様の「(work, am_off, pm_off) を含める」は要再検討（計画上の出勤と実績の二指標に分離する案）。
...
- [ ] フロント: 欠落行に「全休」クイックボタンを追加（override=off_full の即時適用）
- [ ] フロント: 未打刻件数の算出（off/ignore 除外、半休は対象）を Stats に反映
- [ ] 集計仕様の決定: 出勤日数を「実績」と「計画」の二指標に分離するか再定義
