...
## 実装結果サマリ（2025-11-10 クローズ、11-14 追記）

- MVP は実装済み（ステータス帯・クイックアクション・アラートピル/日付一覧・タイムライン・メモ・通知プレビュー）。
- 2025-11-14 追記：
  - アラートから `attendance_list.html?date=...&back=...&backMode=auto` で遷移し、編集完了後に自動戻り（同一オリジンのみ）。
  - 対象日を一覧側で強制可視化＋自動展開、安定後にハイライト解除。
