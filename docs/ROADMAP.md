...
- 完了
  ...
  - ユーザー向けエントリ
    ...
    - My Day ダッシュボード（`myday_dashboard.html`）【完了】
      - 今日の打刻ステータスと警告表示
      - クイックアクション（状態連動）
      - 未読通知バッジと最新 3 件プレビュー
  ...
  - レート制限（SEC-002 完了）
    ...
    - `request_rate_limit` テーブル使用、IP ベースのレート制限機構実装済み
  - アプリ内通知（最小）
    - 決裁時に申請者へ通知レコード生成、ユーザー画面でベル＋モーダル表示
    - 未読 0 件のときは通知ベルを非表示にする挙動に変更
    - 通知の個別既読に対応（モーダル内で各通知に「既読」ボタン、実行後に未読バッジを即時更新）
  - 日ステータス基盤（ATT-001 部分完了）
    - `day_status_effective` ビュー実装（`sql/migrations/2025-11-11_day_status_effective_view.sql`）
...
- 進行中/保留（次段）
  ...
  - 勤怠記録に「休みの明示」記録の完全統合（ATT-001 継続）
    - 基本 UI 実装済み。今後: 過去日の「修正申請」導線、集計ロジックの完全統合
  - CSV/PDF 出力（残高・最短失効・期限間近）【未実装】
  - 一般ユーザーの有給確認 UI 拡張（履歴・残高・最短失効の自己確認）

  - 着手順（メール保留を考慮）
    1. CSV/PDF 出力
    2. セキュリティ/監査の強化（ビュー/エクスポート、RateLimit 可視化の拡充）
    3. 一般ユーザーの有給確認 UI 拡張
...
## 次に実装する 3 件（候補／2025-11-29 更新）

- CSV エクスポート（残高/最短失効、期限間近）【優先度: 高】
  - 対応 Issue: EXP-001
  - `api/paid_leave_expiring_report.php` の作成と `admin.html` へのダウンロードリンク追加
- セキュリティ/監査の強化（継続: 検索・ページング・RateLimit 可視化の拡充）
  - 対応 Issue: 継続タスク
- 一般ユーザーの有給確認 UI 拡張（履歴・残高・最短失効の自己確認）
  - 対応 Issue: USER-UI-001
