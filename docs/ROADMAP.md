...
- 進行中/保留（次段）
  ...
  - ダッシュボードの打刻アラート実装（進行中）
    - 現状: トグル＋期間セレクタ実装済み、上書き日除外、未打刻は過去日のみ、カード単独再描画でちらつき抑制済み。
    - 保留: 「未打刻」の最終判定ルール（休日/公休/休暇/勤務不要日の扱い）確定、サーバー側フィルタへ移行（days/トグルをクエリ化）、パフォーマンス検討。
...
- 一般ユーザーの有給確認 UI 拡張（残高・最短失効・履歴の自己確認）
...
## 引継ぎ用プロンプト（新チャット開始時に貼り付け）

標準版（厳密・推奨）

```md
このプロジェクトの前提と優先度は docs/CONTEXT_SUMMARY.md と docs/ROADMAP.md に記載されています。両ファイルを読み込み、その内容を“単一の真実源”として以後の提案・設計・実装・修正を行ってください。

運用ルール:

- 優先度・着手順・進行状況は ROADMAP.md を優先
- 実装上の前提・UI 仕様・除外条件は CONTEXT_SUMMARY.md を優先
- 衝突があれば、両ファイルの記述を明示して整合案を提案した上で、最小差分で整合を取る
- 破壊的変更は事前に影響範囲・移行手順・ロールバックを提示

期待する振る舞い:

- まず docs/CONTEXT_SUMMARY.md と docs/ROADMAP.md を読み込んだ上で、理解した前提（箇条書き）を短く再提示
- 以後の回答は、編集対象のファイル名・関数名・要素 ID を明示し、要件/制約との対応を簡潔に説明
- 複数案がある場合は、メリデメと採用基準を併記
- 既存 UI/実装の原則（例: ダッシュボードの打刻アラートはトグル/期間セレクタを localStorage で永続化、上書き日は除外、未打刻は過去日のみ、アラートカードのみ再描画でちらつき抑制）は尊重
- 不明点は最小限の確認のみ行い、ブロックしない範囲で合理的に仮定して進める

品質ゲート:

- 変更後はビルド/静的検査（該当範囲）/最小テストを自己実行し、結果を PASS/FAIL で簡潔に報告
- 失敗があれば最大 3 回まで狙いを絞ってリトライし、改善不可なら原因・選択肢・残作業を提示

回答スタイル:

- 日本語、簡潔、具体
- UI 変更は「どの要素をどのように」変えるかを明示
- コード変更は最小差分を原則。既存スタイル/命名/API 互換を尊重
- コマンドは必要時のみ最小限で（pwsh 想定）

まず、docs/CONTEXT_SUMMARY.md と docs/ROADMAP.md を読み込み、理解した主要前提（上位 5〜10 項目）を短く要約してから、次の作業に入ってください。
```

簡易版（短文）

```md
docs/CONTEXT_SUMMARY.md と docs/ROADMAP.md を読み込み、以後は両方の内容を前提にしてください。優先度・着手順は ROADMAP.md、実装前提/UI 仕様は SUMMARY を優先。最初に主要前提を短く要約してから作業に入ってください。
```
